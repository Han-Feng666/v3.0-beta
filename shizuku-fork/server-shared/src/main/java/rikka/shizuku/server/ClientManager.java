...
public class ClientManager<ConfigMgr extends ConfigManager> {
    ...
    public ClientRecord addClient(int uid, int pid, IShizukuApplication client, String packageName, int apiVersion) {
        ...
        // Manager (寒锋自己) 永远视为已授权 —— server 启动发起方, 自带权限
        // 让 requireClient(callingUid, callingPid).allowed 这条 path 始终返回 true, 避免
        // SDK Shizuku.checkSelfPermission 走 requireService().checkSelfPermission 时被拒 IllegalArgumentException
        if (ServerSharedConstants.MANAGER_APPLICATION_ID.equals(packageName)) {
            clientRecord.allowed = true;
        }
