...
public class BinderSender {
    ...
    private static final String PERMISSION_MANAGER = "com.HanFeng.permission.shizuku.MANAGER";
    private static final String PERMISSION = ServerConstants.PERMISSION;
    /**
     * 官方 SDK (dev.rikka.shizuku:api / dev.rikka.shizuku:provider) 的客户端仅声明
     * <uses-permission android:name="moe.shizuku.manager.permission.API_V23" />。
     * 不接收这个旧权限名,第三方 APP 永远收不到 binder,attach 不上、授权链路直接断。
     */
    private static final String PERMISSION_LEGACY_OFFICIAL = ServerConstants.PERMISSION_LEGACY_OFFICIAL;
        ...
        for (String packageName : packages) {
            ...
            if (ArraysKt.contains(pi.requestedPermissions, PERMISSION_MANAGER)) {
                ...
                if (granted) {
                    ...
                    || ArraysKt.contains(pi.requestedPermissions, PERMISSION_LEGACY_OFFICIAL)) {
                LOGGER.i("sendBinder to client %s uid=%d perm=%s", packageName, uid,
                        ArraysKt.contains(pi.requestedPermissions, PERMISSION_LEGACY_OFFICIAL) ? "legacy" : "fork");
