...
public class ShizukuService extends Service<ShizukuUserServiceManager, ShizukuClientManager, ShizukuConfigManager> {
    ...
    public static void main(String[] args) {
        // 入口指纹: 用 System.err.println 打到 stderr (落到 starter.err 文件),
        // 不依赖任何 hidden API. 同步用 android.util.Log.i 写到 logcat tag ShizukuStarter.
        System.err.println(">>> ShizukuService main enter, build 20260721-2");
        try { android.util.Log.i("ShizukuStarter", "ShizukuService main enter, build 20260721-2"); }
        catch (Throwable ignore) {}

        // server 跑在 app_process 下, 没有 application 入口, ART 不会自动豁免 hidden API.
        // 后续 getManagerApplicationInfo -> PackageManagerApis.getApplicationInfoNoThrow
        // 内部走 IActivityManager.getService() 等都是 hidden API,
        // 在 Android 9+ 上会被 restrict 拦下抛 NoSuchMethodError / SecurityException
        // 导致 server 进程直接崩在 onCreate, 8s 内 binder 注册不上.
        // 必须在最早一行调用 addHiddenApiExemptions("L") 把所有 hidden member 放行.
        try {
            boolean ok = org.lsposed.hiddenapibypass.HiddenApiBypass.addHiddenApiExemptions("L");
            System.err.println(">>> HiddenApiBypass ok=" + ok);
            try { android.util.Log.i("ShizukuStarter", "HiddenApiBypass ok=" + ok); } catch (Throwable ignore) {}
        } catch (Throwable t) {
            t.printStackTrace(System.err);
            try { android.util.Log.e("ShizukuStarter", "HiddenApiBypass failed", t); } catch (Throwable ignore) {}
        }
        ...
        try {
            ...
            // 直接写到 /data/local/tmp/hanfeng_shizuku_server.crash 的崩溃栈文件.
            // app_process 内 RuntimeInit 会把 stderr 重定向到 logcat, 之前的 .err 不可靠.
            try {
                java.io.FileWriter fw = new java.io.FileWriter("/data/local/tmp/hanfeng_shizuku_server.crash");
                java.io.PrintWriter pw = new java.io.PrintWriter(fw);
                pw.println("=== ShizukuService crashed in main at " + new java.util.Date() + " ===");
                ...
                pw.println("shizuku.library.path=" + System.getProperty("shizuku.library.path"));
                pw.close();
            } catch (Throwable ignore) {}
            // 也走 try logcat 一道
            try { android.util.Log.e("ShizukuStarter", "ShizukuService crashed in main", t); } catch (Throwable ignore) {}
    ...
    public ShizukuService() {
        ...
        assert ai != null;
        // 仅存 appId (uid 去掉 user 栖位)，以便后续与 UserHandleCompat.getAppId(callingUid) 相同口径比较。
        // 否则 ai.uid=10447 时 getAppId(10447)=447 != 10447，manager 自己调用 updateFlagsForUid/getFlagsForUid 会被拒绝。
    ...
    public void attachApplication(IShizukuApplication application, Bundle args) {
        ...
        if (!isManager) {
            ...
            // Manager (即寒锋自己) attach 时直接给 PERMISSION_GRANTED=true,
            // 避免 SDK 端 permissionGranted=false 导致 checkSelfPermission 走 requireService 路径
            // 抛 IllegalStateException (requireClient 此刻还没注册 clientRecord).
            // manager 本就是 server 启动发起方, 自带权限是合理 design intent.
    ...
    public void dispatchPermissionConfirmationResult(int requestUid, int requestPid, int requestCode, Bundle data) throws RemoteException {
        ...
        if (!onetime && allowed) {
            ...
            for (String packageName : PackageManagerApis.getPackagesForUidNoThrow(requestUid)) {
                ...
                if (pi == null || pi.requestedPermissions == null || !ServerConstants.isClientPermissionRequested(pi.requestedPermissions)) {
                    continue;
                }

                int deviceId = 0;//Context.DEVICE_ID_DEFAULT
                String permToGrant = null;
                for (String perm : ServerConstants.CLIENT_PERMISSIONS) {
                    if (ArraysKt.contains(pi.requestedPermissions, perm)) {
                        permToGrant = perm;
                        break;
                    }
                }
            ...
            for (PackageInfo pi : PackageManagerApis.getInstalledPackagesNoThrow(PackageManager.GET_META_DATA | PackageManager.GET_PERMISSIONS, user)) {
                ...
                if (flags != 0) {
                    list.add(pi);
                        ...
                        && ServerConstants.isClientPermissionRequested(pi.requestedPermissions)) {
                    list.add(pi);
                }
            }
    ...
    private static void sendBinderToClient(Binder binder, int userId) {
        try {
            for (PackageInfo pi : PackageManagerApis.getInstalledPackagesNoThrow(PackageManager.GET_PERMISSIONS, userId)) {
                ...
                if (ServerConstants.isClientPermissionRequested(pi.requestedPermissions)) {
    ...
    static void sendBinderToUserApp(Binder binder, String packageName, int userId, boolean retry) {
        ...
        try {
            ...
            extra.putParcelable("com.HanFeng.shizuku.intent.extra.BINDER", new BinderContainer(binder));
