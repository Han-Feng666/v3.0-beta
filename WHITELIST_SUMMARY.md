...
- 当前版本新增了“疑似影响正常联网规则”检查与展示链路。
- 远程规则源同步或导入后，应用会优先识别更像正常业务域名的候选规则，并向用户展示原因。
- 候选项会附带来源、厂商、业务分类和风险等级，便于用户决定保留或删除。
- 这套流程用于减少消息、支付、音乐、游戏、内容分发等正常业务被误拦截的概率。

## 使用方式

1. 同步远程规则源后，进入相关检查页查看候选规则。
2. 逐条阅读系统给出的影响原因。
3. 根据业务需要决定删除或保留。

## 当前版本稳定性补充

- 规则页读取历史本地规则时，会自动兼容旧版本遗留的空集合字段，避免因为 `removeParamRegexes` 一类字段为空而闪退。
- HTTP 解密路由、HTTPS 解密路由和本地代理共存配置在读取时会自动做最小清洗，降低历史脏数据导致初始化异常的概率。
