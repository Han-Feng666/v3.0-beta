...
- Instructions:
  - 版本号按修改次数递进，例如 `1.6.1` 到 `1.6.2`；补丁位每十次进阶，例如 `1.6.9` 后递进到 `1.7.0`。
...
[MITM 模式积极增强优先]
- Date: 2026-05-27
- Context: 用户明确要求开启 MITM 模式后，以增强拦截效果为首要目标，只要性能占用和网速仍可接受，就不需要保守
- Instructions:
...
[对齐 AdGuard 拦截能力]
- Date: 2026-05-29
- Context: 用户要求只要 AdGuard 能拦截的广告，寒枫也要尽量能拦截
- Instructions:
  - 后续增强优先对齐 AdGuard 有效的能力层，包括规则语义兼容、MITM 响应体识别、HTML cosmetic 清洗和脚本注入处理。
