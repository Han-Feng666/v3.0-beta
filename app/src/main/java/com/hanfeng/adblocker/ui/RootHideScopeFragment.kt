        ...
        loadJob = lifecycleScope.launch {
            try {
                val apps = withContext(Dispatchers.Default) {
                    ...
                    val scopePackages = RootHideRepository.getScopePackages(requireContext())
                    pm.getInstalledPackages(0)
                        .asSequence()
                        .filter { it.packageName != requireContext().packageName && it.applicationInfo != null }
                        .map { info ->
                            InstalledApp(
                ...
                activity?.runOnUiThread {
                    Toast.makeText(requireContext(), "加载应用列表失败: ${e.message}", Toast.LENGTH_LONG).show()
