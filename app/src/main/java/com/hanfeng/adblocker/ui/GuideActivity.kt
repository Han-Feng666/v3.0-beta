...
class GuideActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        private const val DEFAULT_GUIDE_CONTENT =
            "APP作者：寒枫\n" +
            "拦截规则来源：寒枫、大萌主、海哥、不是DD啊、下个ID见、那个谁520，以及 GitHub 开源规则源 EasyList China、StevenBlack/hosts、AdGuardSDNSFilter、EasyPrivacy、OISD、1Hosts、Hagezi DNS Blocklists\n\n" +
            "一、首次使用\n" +
                "1. 第一次打开应用后，建议先阅读本说明，再回到首页点击 \"开启拦截\"。\n" +
                "2. 点击 \"开启拦截\" 后，系统会弹出 VPN 授权提示，这是 Android 的标准安全提示，点击允许后应用才能开始拦截。\n" +
                "3. Android 13 及以上机型建议允许通知权限，方便稳定显示前台服务通知，减少系统清理后台的概率。\n" +
                "4. 部分手机会限制应用列表读取，黑白名单或应用识别不完整时，请到系统设置里手动允许相关权限。\n" +
                "5. 新手建议先只开启基础拦截，确认网络正常后，再按需开启 MITM 模式。\n" +
                "6. 首页状态显示 \"运行中\" 说明拦截已生效；显示 \"恢复中\" 说明服务正在自动重连；显示 \"未开启\" 说明当前没有启动拦截。\n\n" +
                "二、拦截核心效果\n" +
                "现在的拦截效果分为三个层级，从基础的域名拦截到增强过滤，覆盖了绝大多数广告场景：\n\n" +
                "1. DNS 层拦截（基础防线）- 极速、低耗\n" +
                "- 效果：App 请求已知广告域名时会优先在 DNS 阶段被拦截，资源消耗极低。\n" +
                "- 优势：速度快，不费电。\n" +
                "- 增强：已补强 DoH、DoT、DoQ、HTTPDNS 等加密 DNS / 反绕过目标的识别与阻断。\n" +
                "- 局限：对于完全自定义协议、私有隧道或未命中规则的硬编码目标，仍需依赖下一级增强过滤。\n\n" +
                "2. MITM 增强过滤（核心防线）- 穿透隐身广告\n" +
                "- 拦截目标：HTTPS 加密广告、HTTPDNS 劫持过来的广告、HTTP/2 隐藏广告。\n" +
                "- 工作原理：拦截器会在中间识别目标流量，检查请求路径（如 /ad/, /banner/）和响应中的广告特征。\n" +
                "- 精准清洗：发现广告特征后，会对命中的 HTTP/1.1、HTTP/2 流量做拦截、参数清洗、路径命中和响应替换处理。\n" +
                "- QUIC/HTTP3 精细处理：结合 DNS 命中域名、别名目标、广告规则、小说专项信号和加密 DNS 反绕过目标处理 UDP/443，广告目标会被阻断，必要时推动回退到 TCP 后再进入增强过滤。\n\n" +
                "3. 小说 App 专项净化（难点攻克）\n" +
                "- 拦截目标：番茄、七猫、掌阅、书旗、QQ 阅读等小说 App 的开屏广告、底部悬浮 Banner、福利页广告和激励视频。\n" +
                "- 效果：即使广告域名伪装在主业务域名下（“保护域名”），只要域名、别名链路、URL 路径或响应内容带广告特征也会优先处理。\n" +
                "- QUIC/HTTP3：小说 App 的可疑广告目标会优先阻断 UDP/443，并推动回退到 TCP 后继续增强过滤。\n\n" +
                "三、MITM 模式说明\n" +
                ...
                "   - QUIC/HTTP3：按域名、别名、规则和小说专项信号精细处理，广告目标阻断并推动回退到 TCP\n" +
                "3. 当前 HTTP/3/QUIC 的实现重点是识别、阻断和回退，优先解决广告流量与加密 DNS 绕过问题。\n" +
                "4. 关闭 MITM 模式的优点：速度更稳、兼容性更高、对大多数正常联网场景影响最小。\n" +
                "5. 开启 MITM 模式的优点：除了域名拦截，还会继续识别请求路径、查询参数、响应头、响应体、部分 third-party/domain/denyallow 上下文规则，对开屏广告、激励视频、广告 JSON 处理更强。\n" +
                "6. 对登录、账号、认证一类敏感域名，当前会优先保持兼容，避免因增强过滤影响登录和正常联网。\n" +
                "7. 开启前请先安装证书。若开启后遇到异常，关闭开关并重试可恢复标准模式。\n\n" +
                "四、性能与发热优化\n" +
                "1. 普通流量优先直通，只对命中广告规则、命中小说强拦目标、命中加密 DNS 反绕过目标或需要增强过滤的流量做深度处理。\n" +
            ...
            "五、证书安装与管理\n" +
                ...
                "4. 安装完成后回到应用首页，确认状态文字里的证书状态已经更新。\n" +
                "5. 只有在你主动开启 MITM 模式时才需要安装该证书。\n\n" +
            "六、主界面按钮说明\n" +
                "1. \"开启拦截 / 停止拦截\"：启动或停止本地 VPN 拦截服务。\n" +
                "2. \"MITM 模式\"开关：开启后进入增强拦截模式，并导出证书文件供手动安装。\n" +
                "3. \"使用说明\"：打开当前这份帮助文档。\n" +
                "4. \"黑白名单\"：进入应用白名单页面，加入白名单的应用会完全放行。\n" +
                "5. 状态卡片：显示当前工作状态、拦截模式、证书状态和版本号，当前版本为 4.0-beta。\n" +
                "6. 如果你没有手动关闭拦截，但首页短暂显示 \"恢复中\"，说明应用检测到服务异常后正在自动恢复。\n\n" +
                "七、规则页按钮说明\n" +
                "1. \"添加规则\"：把输入框中的域名或规则文本加入规则库。\n" +
                "2. \"粘贴\"：把剪贴板内容追加到规则输入框。\n" +
                "3. \"清空\"：清空当前输入框内容。\n" +
                "4. \"导入规则\"：可从手机本地选择规则文件并导入。\n" +
                "5. \"办卡\"：打开预设外部办卡页面。\n" +
                ...
                "8. 已支持部分 URL / MITM 增强规则，例如路径规则、`removeparam`、`csp`、部分 `third-party`、`domain`、`denyallow`。\n" +
                "9. 对脚本注入、完整重定向语义、复杂逻辑组合、IP-CIDR、GEOIP、GEOSITE 一类当前架构难以安全落地的规则，会自动跳过，并在导入分析页展示。\n" +
                "10. 当前内置规则已加入更多 GitHub 开源广告规则源筛选出的明确广告域名，升级版本后会自动补充导入。\n\n" +
                "九、统计页按钮说明\n" +
            ...
            "十、排查建议\n" +
                ...
                "3. 小说 App 仍有漏网广告时，优先重开应用并重新触发广告位，因为很多广告会在 QUIC 被阻断后回退到 TCP 再进入增强过滤。\n" +
                "4. 某些旧广告通常和本地缓存有关，建议清理对应应用缓存、重开应用后再测试。\n" +
                "5. 如果没有手动关闭拦截，但状态从运行中变成恢复中，先等待 2 到 3 秒，通常会自动恢复。\n" +
                "6. 如果经常自动恢复或被系统中断，请到系统设置把本应用加入省电白名单，允许后台运行，并锁定前台通知。\n" +
                "7. 常见系统入口包括电池优化、自启动管理、后台高耗电限制、后台弹出界面和应用待机。\n" +
                "8. 若恢复后仍无网络，先停止拦截再重新开启；若问题只出现在单个 App，请先加入白名单排查。\n" +
                "9. 如有 BUG 或建议，可通过左界面的\"加群反馈\"入口提交。"
