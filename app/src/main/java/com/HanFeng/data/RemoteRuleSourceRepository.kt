...
object RemoteRuleSourceRepository {
    ...
    private const val KEY_PREFIX_ETAG = "etag_"
    private const val KEY_PREFIX_LAST_MODIFIED = "lastmod_"
    ...
    private const val MAX_TEXT_DOWNLOAD_BYTES = 8 * 1024 * 1024
    private const val DOWNLOAD_TRUNCATION_WARN_BYTES = 7 * 1024 * 1024
    ...
    suspend fun syncSource(
        ...
        runCatching {
            ...
            if (downloadResult.notModified) {
                ...
                onDetailedProgress?.invoke(RemoteRuleSyncProgress(1, 1, updatedSource, RemoteRuleSyncProgress.Stage.COMPLETED, bytesRead = 0L, addedCount = source.lastRuleCount))
                return@runCatching RemoteRuleSyncResult(
                    source = updatedSource,
                    success = true,
                    addedCount = source.lastRuleCount,
                    notModified = true
                )
            }
            val tempFile = downloadResult.file
            downloadResult.etag?.let { newEtag ->
                persistSyncCache(context, source.id, newEtag, downloadResult.lastModified)
            } ?: downloadResult.lastModified?.let { newLM ->
                persistSyncCache(context, source.id, null, newLM)
            }
            truncated = truncated || (downloadedBytes >= DOWNLOAD_TRUNCATION_WARN_BYTES)
            ...
            if (addedCount <= 0) {
                ...
                LogRepository.append(context, "规则源同步完成：${source.name}，导${addedCount}条（疑似截断:${formatBytes(downloadedBytes)}），耗时${totalTime / 1000}秒")
            } else {
            ...
            RemoteRuleSyncResult(
                ...
                errorMessage = when {
                    addedCount <= 0 -> "未导入任何规则 (格式问题)"
                    truncated -> "导入成功但文件大小可能达到下载上限，已截断"
                    else -> null
                }
            )
        }.getOrElse { error ->
    ...
    private data class ConditionalDownloadResult(
    ...
    private fun downloadToFileConditional(
        ...
        try {
            val responseCode = withConditionalRetry(context, url, savedEtag, savedLastModified) { connection, isCachedRequest ->
                val code = connection.responseCode
                if (code == 304) {
                    return@withConditionalRetry ConditionalDownloadResult(file = tempFile, notModified = true)
                }
                if (code !in 200..299) {
                    val errorText = runCatching {
                        connection.errorStream?.readTextPrefix(200)
                    }.getOrNull().orEmpty().ifBlank { "HTTP $code" }
                    throw IOException("规则源下载失败：HTTP $code ${errorText.trim()}")
                }

                val newEtag = connection.getHeaderField("ETag")?.takeIf { it.isNotBlank() }
                val newLastModified = connection.getHeaderField("Last-Modified")?.takeIf { it.isNotBlank() }
                ...
                var truncatedNow = false
                onProgress?.invoke(0L, contentLength, false)
                tempFile.outputStream().use { outputStream ->
                    connection.inputStream.use { inputStream ->
                        while (inputStream.read(buffer).also { bytesRead = it } != -1) {
                            outputStream.write(buffer, 0, bytesRead)
                            totalBytes += bytesRead
                            if (totalBytes > MAX_TEXT_DOWNLOAD_BYTES) {
                                truncatedNow = true
                                break
                            }
                            ...
                            if (now - lastProgressAt >= 500L || contentLength != null && totalBytes >= contentLength) {
                                lastProgressAt = now
                                onProgress?.invoke(totalBytes, contentLength, truncatedNow)
                            }
                        }
                    }
                }
                ...
                if (truncatedNow) {
                    LogRepository.append(context, "规则源下载触发 ${MAX_TEXT_DOWNLOAD_BYTES / 1024 / 1024}MB 上限：$url")
                }
    ...
    private inline fun <T> withConditionalRetry(
        context: Context,
        url: String,
        savedEtag: String?,
        savedLastModified: String?,
        block: (HttpURLConnection, Boolean) -> T
    ): T {
        ...
        listOf(true, false).forEach { preferNonVpnNetwork ->
            ...
            savedEtag?.takeIf { it.isNotBlank() }?.let { connection.setRequestProperty("If-None-Match", it) }
            savedLastModified?.takeIf { it.isNotBlank() }?.let { connection.setRequestProperty("If-Modified-Since", it) }
            try {
                return block(connection, preferNonVpnNetwork)
    ...
    private fun etagPrefsKey(sourceId: String) = "$KEY_PREFIX_ETAG$sourceId"
    private fun lastModifiedPrefsKey(sourceId: String) = "$KEY_PREFIX_LAST_MODIFIED$sourceId"

    private fun loadSyncCache(context: Context, sourceId: String): Pair<String?, String?> {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val etag = prefs.getString(etagPrefsKey(sourceId), null)
        val lastModified = prefs.getString(lastModifiedPrefsKey(sourceId), null)
        return etag?.takeIf { it.isNotBlank() } to lastModified?.takeIf { it.isNotBlank() }
    }

    private fun persistSyncCache(
        context: Context,
        sourceId: String,
        etag: String?,
        lastModified: String?
        ...
        if (etag.isNullOrBlank()) prefs.remove(etagPrefsKey(sourceId)) else prefs.putString(etagPrefsKey(sourceId), etag)
        if (lastModified.isNullOrBlank()) prefs.remove(lastModifiedPrefsKey(sourceId)) else prefs.putString(lastModifiedPrefsKey(sourceId), lastModified)
        prefs.apply()
    }
}
